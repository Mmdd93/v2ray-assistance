...
# Function to create an IPIP tunnel with IPv4
create_ipip_tunnel_ipv4() {
    ...
    if [[ ! "$service_name" =~ ^ipip- ]]; then
        service_name="ipip-$service_name"
    fi
    ...
    # Generate the systemd service file for IPIP tunnel
    echo -e "\n${greEN}Creating systemd service file for $service_name...${RESET}"
    cat <<EOF > "$service_file"
[Unit]
Description=IPIP Tunnel $service_name
After=network.target

[Service]
Type=oneshot
ExecStart=/usr/bin/env sh -c '\
    /sbin/ip tunnel add $service_name mode ipip local $local_ip remote $remote_ip ttl 255 && \
...
EOF
    ...
    echo -e "\n${greEN}IPIP Tunnel $service_name created.${RESET}"
    read -p "Press Enter to continue..."
}

# Function to create an IPIP tunnel with IPv6 over IPv4


# Function to manage IPIP tunnels (similar to your manage_tunnels but for IPIP)
manage_ipip_tunnels() {
    # List all available IPIP tunnel services
    echo -e "${greEN}Available IPIP tunnels:${RESET}"
    local tunnels=()

    # Get all active IPIP tunnel services
    for dir in /usr/lib/systemd/system; do
        for file in "$dir"/ipip-*.service; do
            if [[ -f "$file" ]]; then
    ...
    if [[ ${#tunnels[@]} -eq 0 ]]; then
        echo -e "${RED}No active IPIP tunnels found.${RESET}"
        read -p "Press Enter to continue..."
        return 1
    fi
...
# Function to stop all IPIP tunnel services
stop_all_ipip_tunnels() {
    echo -e "${greEN}Stopping all IPIP tunnel services...${RESET}"
    ...
    if [[ $stopped_any == false ]]; then
        echo -e "${RED}No active IPIP tunnel services found to stop.${RESET}"
        return 1
    fi

    echo -e "${greEN}All IPIP tunnel services have been stopped.${RESET}"
    read -p "Press Enter to continue..."
}

# Function to enable and start all IPIP tunnel services
enable_and_start_ipip_tunnels() {
    echo -e "${greEN}Enabling and starting all IPIP tunnel services...${RESET}"
    ...
    if [[ $started_any == false ]]; then
        echo -e "${RED}No IPIP tunnel services found to enable or start.${RESET}"
        return 1
    fi

    echo -e "${greEN}All IPIP tunnel services have been enabled and started.${RESET}"
    read -p "Press Enter to continue..."
}

# Function to restart all IPIP tunnel services
restart_ipip_tunnels() {
    echo -e "${greEN}Restarting all IPIP tunnel services...${RESET}"
    ...
    if [[ $restarted_any == false ]]; then
        echo -e "${RED}No IPIP tunnel services found to restart.${RESET}"
        return 1
    fi

    echo -e "${greEN}All IPIP tunnel services have been restarted.${RESET}"
    read -p "Press Enter to continue..."
}
...
while true; do
    ...
    echo -e "      \033[1;32mIPIP Tunnel Service Method\033[0m"
    echo -e "\033[1;34m=========================================\033[0m"
    echo -e "\033[1;36m 1.\033[0m \033[1;32mCreate IPIP Tunnel IPv4\033[0m"
    echo -e "\033[1;36m 2.\033[0m \033[1;32mManage IPIP Tunnels\033[0m"
    echo -e "\033[1;36m 3.\033[0m \033[1;32mStart all IPIP Tunnels\033[0m"
    echo -e "\033[1;36m 4.\033[0m \033[1;32mStop all IPIP Tunnels\033[0m"
    echo -e "\033[1;36m 5.\033[0m \033[1;32mRestart all IPIP Tunnels\033[0m"
